...
open class ListAdapterSearch<T>(@LayoutRes layout: Int, viewModel: ViewModelPage) : ListAdapter<T>(layout, viewModel) {
    ...
    private val strings = hashMapOf(
        API_KEY_SELF to R.string.search_self,
        "findAllByUserId" to R.string.search_findAllByUserId
    )
    ...
    private fun setChip(key: String, holder: View) {
        holder.apply {
            ...
            if (strings.containsKey(key)) {
                chip.text = context.getString(strings.getValue(key))
            } else {
                chip.text = key
            }
